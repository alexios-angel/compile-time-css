...
constexpr bool match_at(const selector_view & sel, std::size_t si, const element_ref * chain,
                        std::size_t ci) noexcept {
...
CTLL_EXPORT template <typename... Steps, std::size_t N>
constexpr bool matches(selector<Steps...> s, const element_ref (&chain)[N]) noexcept {
	return detail::matches_view(view_of(s), chain, N);
}
CTLL_EXPORT template <typename... Steps>
constexpr bool matches(selector<Steps...> s, const element_ref * chain, std::size_t n) noexcept {
	return detail::matches_view(view_of(s), chain, n);
}
...
struct rule_shape<rule<ctll::list<Sels...>, ctll::list<Decls...>>> {
	static constexpr std::size_t entries = sizeof...(Sels) * sizeof...(Decls);

	template <typename Out>
	static constexpr void fill(Out & out, std::size_t & at, std::int32_t & order) noexcept {
		(fill_selector<Sels>(out, at, order), ...);
		order += static_cast<std::int32_t>(sizeof...(Decls));
	}
	template <typename Sel, typename Out>
	static constexpr void fill_selector(Out & out, std::size_t & at, std::int32_t order) noexcept {
...
template <typename... Rules> struct sheet_data<stylesheet<Rules...>> {
	static constexpr std::size_t count = (rule_shape<Rules>::entries + ... + 0);
...
CTLL_EXPORT template <typename... Rules>
constexpr std::size_t entry_count(stylesheet<Rules...>) noexcept {
...
constexpr std::string_view query(stylesheet<Rules...> sheet, const element_ref * chain,
	...
	for (std::size_t i = 0; i < entry_count(sheet); ++i) {
...
CTLL_EXPORT template <typename... Rules, std::size_t N>
