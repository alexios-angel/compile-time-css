...
// --- tag classification (case-insensitive; builder names are already
// canonical lowercase, validator names arrive as written)
...
// elements that live in <head>: seeing anything else there moves
// insertion to <body>
...
// end tags the spec lets a document omit; a close tag (and </body>,
// </html> and EOF) may close through these silently
...
// does an incoming open tag t auto-close a still-open element? (the
// HTML5 optional-end-tag table, applied at the top of the open stack)
...
// --- the validator: chunks lowered to plain values, walked once
